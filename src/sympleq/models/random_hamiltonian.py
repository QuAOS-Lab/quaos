...
def random_gate_symmetric_hamiltonian(G: 'Gate',
    ...
    def _gate_order(gate: Gate, max_iter: int = 256) -> int:
        """Return the smallest k > 0 such that gate^k = I (symplectic part), or max_iter if not found."""
        n = gate.n_qudits
        L = int(np.lcm.reduce(gate.dimensions))
        C = gate.symplectic % L
        cur = np.eye(2 * n, dtype=int)
        for k in range(1, max_iter + 1):
            cur = (cur @ C) % L
            if np.array_equal(cur % L, np.eye(2 * n, dtype=int)):
                return k
        return max_iter
