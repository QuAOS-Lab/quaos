...
def calculate_mean_estimate(data: np.ndarray, weights: np.ndarray) -> float:
    """
    Calculate the mean estimate of the Hamiltonian/observable from the data generated by aquire.

    Parameters
    ----------
    data : np.ndarray
        A 3D array of shape (p, p, d) where p is the number of Pauli strings and d is the number of outcomes.
    weights : np.ndarray
        A 1D array of length p where each element is a weight for the corresponding Pauli string.

    Returns
    -------
    float
        The mean estimate of the Hamiltonian/observable.
    """
    p = data.shape[0]
    d = data.shape[2]
    mean = 0
...
def calculate_statistical_variance_estimate(covariance_graph: graph, scaling_matrix: np.ndarray) -> float:
    """
    Calculate the statistical variance estimate of the Hamiltonian/observable from the covariance graph generated by
    aquire and the Bayesian Covariance Graph.

    Parameters
    ----------
    covariance_graph : graph
        A graph representing the covariance between Pauli strings.
    scaling_matrix : np.ndarray
        A 2D array of shape (p, p) where p is the number of Pauli strings and each element is a weight for the
        corresponding Pauli strings.

    Returns
    -------
    float
        The statistical variance estimate of the Hamiltonian/observable.
    """
...
def calculate_systematic_variance_estimate(data: np.ndarray, weights: np.ndarray, diagnostic_data: np.ndarray) -> float:
    """
    Calculate the systematic variance (hardware error) estimate of the Hamiltonian/observable from the data generated
    by aquire and the diagnostic data.
    ...
    diagnostic_data : np.ndarray
        A 2D array of shape (p, 2) where p is the number of Pauli strings and each element is the number of '0' and '1'
        outcomes in the diagnostic data.

    Returns
    -------
    float
        The systematic variance estimate of the Hamiltonian/observable.
    """
    p = data.shape[0]
    d = data.shape[2]
    # estimate w_i
...
def true_statistical_variance(H: PauliSum, psi: np.ndarray, S: np.ndarray) -> float:
    ...
    ----------
    H : PauliSum
        The Hamiltonian/observable to be simulated.
    psi : np.ndarray | None
        The state vector to simulate the average of the observable. If None, comparison of experimentally estimated
        values to true values is not possible.
    S : np.ndarray
        The scaling matrix for the covariance graph.
