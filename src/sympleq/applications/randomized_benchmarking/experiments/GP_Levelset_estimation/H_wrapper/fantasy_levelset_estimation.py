...
_SELECTED_CONFIG_METADATA: dict[RMBConfig, dict] = {}
...
def valid_config(config: RMBConfig) -> bool:
    ...
    required_1q = h_wrapper_1q_gates(config.n_qubits)
    if bool(config.use_scrambler):
        required_1q += 2 * int(config.n_qubits)
    return int(config.n_1qb_gates) >= required_1q
...
def raw_validity(
    ...
    required_1q = h_wrapper_1q_gates(q)
    if settings.use_scrambler:
        required_1q += 2 * q
    return n_1q >= required_1q, n_1q, n_2q, q


def record_selected_config_metadata(
    config: RMBConfig,
    *,
    source: str,
    raw_n_gates: float,
    ...
    _SELECTED_CONFIG_METADATA[config] = {
        "source": source,
        "raw_gates": float(raw_n_gates),
        "raw_ratio": float(raw_ratio),
        "raw_q": int(raw_q),
        "raw_n_1q": int(raw_n_1q),
        "raw_n_2q": int(raw_n_2q),
        "raw_valid": bool(raw_is_valid),
        "repaired": not bool(raw_is_valid),
        "chosen_n_1q": int(config.n_1qb_gates),
        "chosen_n_2q": int(config.n_2qb_gates),
        "chosen_gates": int(config.n_gates),
        "chosen_ratio": float(config.ratio_2_qb_gates),
        "chosen_q": int(config.n_qubits),
    }


def selected_config_metadata(config: RMBConfig) -> dict | None:
    metadata = _SELECTED_CONFIG_METADATA.get(config)
...
def reset_repair_stats() -> None:
    ...
    _SELECTED_CONFIG_METADATA.clear()
...
def sobol_initial_candidates(
    ...
    for u_n, u_r in unit_points:
        ...
        raw_is_valid, raw_n_1q, raw_n_2q, raw_q = raw_validity(settings, n_gates, ratio)
        config = settings.make_config(n_gates, ratio)

        # Realized gate-count rounding can create duplicates.
        if config not in seen:
            record_repair(raw_is_valid)
            record_selected_config_metadata(
                config,
                source="sobol",
                raw_n_gates=n_gates,
                raw_ratio=ratio,
...
def select_fantasy_globalsur_batch(
    ...
    *,
    ...
    while len(selected) < batch_limit and attempts < max_attempts:
        ...
        record_selected_config_metadata(
            candidate,
            source="fantasy_globalsur",
            raw_n_gates=raw_n_gates,
            raw_ratio=raw_ratio,
            raw_is_valid=raw_is_valid,
            raw_n_1q=raw_n_1q,
            raw_n_2q=raw_n_2q,
