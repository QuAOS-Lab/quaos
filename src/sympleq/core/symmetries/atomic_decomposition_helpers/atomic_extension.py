# sympleq/core/symmetries/atomic_decomposition_helpers/atomic_extension.py
...
from ..modular_helpers import mod_p, inv_mod_mat, rank_mod, independent_columns, mat_pow_mod
...
def field_scalar_apply_to_vector(F: np.ndarray, field: GFpExtension, scalar: Sequence[int] | int, v: np.ndarray, p: int) -> np.ndarray:
...
def h_pair(field: GFpExtension, H: np.ndarray, x: Sequence[Sequence[int] | int], y: Sequence[Sequence[int] | int]) -> Element:
    m = H.shape[0]
    acc = field.zero
    for i in range(m):
...
def _vec_add_scaled(field: GFpExtension, x: List[Element], y: Sequence[Element], a: Sequence[int] | int) -> List[Element]:
    aa = field._coerce(a)
