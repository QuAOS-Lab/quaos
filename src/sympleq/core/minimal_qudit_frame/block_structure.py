...
def block_decompose(F: np.ndarray, p: int, **kwargs):
    ...
    """
    return minimal_qudit_frame(F, p, **kwargs)
...
def block_decompose_certified(
    ...
    **kwargs,
    ...
    Returns ``(Sigma, B, info)`` and raises ``MinimalQuditFrameCertificationError`` unless the
    result is a certified MQF decomposition.  By default it also requires the
    qudit cost to be certified minimal; set ``require_minimal=False`` to accept
    certified MQF decompositions whose minimality is not certified.
    """
    return minimal_qudit_frame_or_raise(F, p, require_minimal=require_minimal, **kwargs)
...
def block_decompose_optimal(F: np.ndarray, p: int, **kwargs):
    ...
    if "require_minimal" in kwargs:
        raise TypeError(
            "block_decompose_optimal always requires certified minimality; "
            "use block_decompose_certified for require_minimal=False."
        )
    return minimal_qudit_frame_or_raise(F, p, require_minimal=True, **kwargs)


def qudit_coupling_graph(F: np.ndarray) -> dict[int, set[int]]:
    """
    Construct the undirected qudit-coupling graph of a 2n x 2n symplectic matrix.
    """
    F = np.asarray(F)
    ...
    for i in range(n):
        for col_idx in (i, n + i):
            ...
            z_part = col[n:]
            for j in range(n):
                if (x_part[j] != 0 or z_part[j] != 0) and j != i:
                    neighbors[i].add(j)
                    neighbors[j].add(i)
    return neighbors
