...
from sympleq.core.symmetries.modular_helpers import mod_p, inv_mod_mat, is_symplectic, rank_mod, nullspace_mod
from .mqf_linear import darboux_basis_from_span
...
def canonical_V_beta_block_p2(length: int, beta: int = 1) -> np.ndarray:
    ...
    a canonical representative in its natural basis and conjugates the preserved
    alternating form to the standard symplectic form.
    """
    L = int(length)
    b = int(beta) & 1
    if L <= 0 or (L % 2):
    ...
    if not np.array_equal(mod_p(U.T @ G @ U, 2), G % 2):
        raise RuntimeError("internal error: canonical V_beta block does not preserve its defining symplectic form")
...
def canonical_W_beta_block_p2(length: int, beta: int = 1) -> np.ndarray:
    ...
    nonzero beta parameter is beta=1.  beta=0 is the ordinary W(length) block.
    """
    L = int(length)
    b = int(beta) & 1
    if L <= 0 or L % 2 == 0:
    ...
    # Symplectic form in the canonical basis: (w_i, x_{-i}) = 1.
    G = np.zeros((dim, dim), dtype=np.int64)
    for i in idxs:
        a = pos("w", i)
        c = pos("x", -i)
    ...
    # w_i images, using the corrected W_beta recurrence.
    for i in idxs:
        if i == 2 * ell:
            add_image("w", i, "w", i)
    ...
    U %= 2
    if not np.array_equal(mod_p(U.T @ G @ U, 2), G % 2):
        raise RuntimeError("internal error: canonical W_beta block does not preserve its defining symplectic form")

    # Columns of S are a Darboux basis for G: S^T G S = Omega.  Convert the
    # canonical representative into standard grouped symplectic coordinates.
    S = darboux_basis_from_span(G, np.eye(dim, dtype=np.int64), 2)
    Fstd = mod_p(inv_mod_mat(S, 2) @ U @ S, 2)
    if not is_symplectic(Fstd, 2):
...
def random_symplectic_matrix_p2(n: int, *, seed: int = 0, steps: int = 32) -> np.ndarray:
    ...
    for _ in range(int(steps)):
        ...
        if kind == 0:
            ...
            E = np.block([[np.eye(n, dtype=np.int64), Sym], [np.zeros(
                (n, n), dtype=np.int64), np.eye(n, dtype=np.int64)]]) % 2
        else:
            R = rng.integers(0, 2, size=(n, n), dtype=np.int64)
            Sym = (R + R.T) % 2
            # Same characteristic-2 diagonal correction for the lower shear block.
            np.fill_diagonal(Sym, rng.integers(0, 2, size=n, dtype=np.int64))
            E = np.block([[np.eye(n, dtype=np.int64), np.zeros((n, n), dtype=np.int64)],
                         [Sym, np.eye(n, dtype=np.int64)]]) % 2
        S = (S @ E) % 2
