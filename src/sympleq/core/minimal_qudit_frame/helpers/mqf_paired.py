...
def mqf_blocks_in_paired_sector(
    F: np.ndarray,
    p: int,
    key: tuple[int, ...],
    key_star: tuple[int, ...],
    ...
    """
    MQF block construction for paired sector W = V_q ⊕ V_{q*} (q != q*).
    ...
    # Here we instead extract one MQF block at a time and remove its symplectic orthogonal
    ...
    try:
        ...
        while rem_dim > 0:
            ...
            if not extracted:
                message = "Paired sector: could not extract a valid block from remaining subspace"
                if last_err:
                    message += f" (last_err={last_err})"
                raise ExtractionObstruction(message)
    ...
    inv = MQFInvariant(sector_key=key, sector_type="paired", poly_key=key, data=inv_data)
    blocks = [MQFBlock(b.T_blk, b.half_dim, b.sector_key, inv) for b in blocks]
