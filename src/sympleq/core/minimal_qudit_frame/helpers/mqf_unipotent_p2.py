...
from .mqf_types import (MQFBlock, MQFInvariant,
                        ExtractionObstruction, SearchBudgetExceeded)
from .mqf_linear import (
    restrict_operator,
    is_nondegenerate,
...
from .mqf_krylov import select_module_generators_from_top_quotient
from .mqf_filtration import build_nilpotent_filtration
...
def _mid_exponent_p2(L: int) -> int:
    """
    Exponent used by the binary unipotent quadratic refinement.
...
def _beta_from_top_generators_p2(
    ...
    """
    Beta label of an extracted p=2 indecomposable.
...
def _top_coeff_candidates(m: int, *, exhaustive_limit: int = 10) -> List[np.ndarray]:
    ...
    MQF top pieces encountered after repeated extraction.
    """
    m = int(m)
    if m <= 0:
        return []
...
def _p2_full_sector_invariants(N, Omega, F_sec, deg_q, max_exp0, m, p):
    """Compute binary unipotent sector invariants before extraction.

    The kernel profile, per-length top-form invariants, and generator-count
    diagnostics determine the V/W type at each length and feed the cost
    certificate.
    """
...
def _p2_extract_blocks(F_sec, N, Omega, T_sec, m, max_exp0, deg_q, key, p):
    """Type-by-invariant extraction engine for a binary unipotent sector.

    Peels the longest remaining length one block at a time. ``accept_block``
    records a lifted, verified summand with direct nondegeneracy and invariance
    checks. ``try_invariant_candidates`` dispatches from the top pairing to
    V-only or W-only candidate families. Returns the blocks, their
    sector-coordinate bases, per-block metadata, and the extraction log.
    """
    ...
    def accept_block(span: np.ndarray, top_gens: np.ndarray, L: int, typ_base: str) -> bool:
        ...
        if typ_base == "V":
            ...
            # odd-length quadratic refinement when L is odd.
        ...
        blocks.append(MQFBlock(T_blk=T_blk_amb, half_dim=int(T_blk_amb.shape[1] // 2), sector_key=key, inv=None))
    ...
    def try_invariant_candidates(A_top: np.ndarray, L: int) -> bool:
        """
        Select and lift a top-space candidate dictated by the top pairing.

        Dispatch is *mandatory* on the top form b_L:
          * even L, b_L non-alternating (diag != 0): extract V-blocks only, by
            orthonormalising the top form and lifting each anisotropic line.
            There is NO fall-through to W-pairs: a W here would have
            half-dimension L instead of L/2 and is therefore cost-pessimal.
        ...
        Acceptance carries a one-scalar top-pairing certificate
        (b_L(v,v)=1 for a V-line; b_L(v,w)=1 with the top Gram nonsingular for a
        W-pair). The direct nondegeneracy/invariance test in accept_block is
        retained as a defensive runtime assertion. No arbitrary coefficient
        enumeration is used on the certified path.
        """
        L = int(L)
        Btop = _top_pairing_matrix(A_top, Omega, N, L)
        invL = _p2_length_form_invariants(A_top, Omega, N, L)

        # On a nondegenerate sector the top form is nondegenerate, i.e.
        # rad b_L = 0. A nonzero radical means an invariant was computed wrong;
        # surface it rather than extracting a wrong block.
        if int(invL["rad_dim"]) != 0:
            raise ExtractionObstruction(
                f"p=2 unipotent: top form b_L has nonzero radical at L={L} "
                f"(rad_dim={invL['rad_dim']}); expected a nondegenerate top form."
            )
        ...
        if L % 2 == 0 and not b_alt:
            ...
            for c in _anisotropic_orthogonal_line_candidates(Btop):
                ...
                if accept_block(Cv, v_top, L, "V"):
                    return True
            # A non-alternating top form should provide anisotropic lines. If
            # none lift, surface the obstruction instead of falling back to a
            # cost-pessimal W-pair.
            return False
        ...
        for ce, cf in _hyperbolic_pair_candidates(Btop):
            ...
            # Need b_L(v,w)=1 and a nonsingular 2x2 top Gram, i.e. NOT both
            # lines anisotropic (that would be two V-lines, not a W-pair).
...
def _p2_verify_kernel_profile(built_cols_sec, N, kernel_profile, max_exp0, m, p, key):
    """Assert that extracted blocks reproduce the sector kernel profile.
    ...
    """
    dim_sector = 2 * m
    all_cols_sec = np.concatenate(built_cols_sec, axis=1) if built_cols_sec else np.zeros(
        (dim_sector, 0), dtype=np.int64)
...
def _p2_build_certificate(blocks_meta, kernel_profile, kernel_profile_blocks,
                          ...
                          deg_q, max_exp0):
    """Assemble the cost certificate and invariant payload.
    ...
    binary unipotent sector bound by the sector dispatch layer.
    """
    ...
    unimplemented = sorted({str(b.get("type", ""))
                           for b in blocks_meta if str(b.get("type", "")) not in implemented_types})
    ...
    inv_data: Dict[str, Any] = {
        ...
        "p2_unipotent": {
            ...
            "classification_status": (
                "implemented extraction for W(k), V_beta(2k), and W_beta(2l+1) succeeded"
        ...
        "cost_certificate": {
            ...
            "note": (
                "p=2 unipotent sector certified using implemented W(k), V_beta(2k), "
                "and W_beta(2l+1) quotient-top extraction."
                if implemented_complete else
...
def mqf_blocks_in_unipotent_self_sector_p2(
    F: np.ndarray,
    key: Tuple[int, ...],
    primaries: dict,
    ...
    Deterministic type-by-invariant policy (no search/fallback):
      * at each top length L, dispatch on the top form b_L -- V-blocks when it is
        non-alternating, W-pairs when alternating or L is odd;
    ...
    if V.shape[1] == 0:
        inv = MQFInvariant(sector_key=key, sector_type="self", poly_key=key, data={"status": "OK", "empty": True})
        return [], inv
    ...
    # Compute invariants before extraction so the certificate is independent of
    # the extracted block family.
    kernel_profile, length_summary, length_invariants = _p2_full_sector_invariants(
        N, Omega, F_sec, deg_q, max_exp0, m, p
    )

    # Extract blocks through the deterministic top-form dispatch.
    blocks, built_cols_sec, blocks_meta, extraction_log = _p2_extract_blocks(
        F_sec, N, Omega, T_sec, m, max_exp0, deg_q, key, p
    )

    # Verify the blocks reproduce the sector kernel profile and span.
    ...
    inv = MQFInvariant(sector_key=key, sector_type="self", poly_key=key, data=inv_data)
    blocks = [MQFBlock(b.T_blk, b.half_dim, key, inv) for b in blocks]
    return blocks, inv
