...
invariants alone before any block extraction, so that ``certified_minimal``
becomes the genuine statement
...
from sympleq.core.symmetries.polynomials_fp import poly_monic
...
def sector_lower_bound(
    ...
    The cases below separate paired sectors, non-unipotent self-reciprocal
    sectors, odd-prime x±1 sectors, and binary unipotent sectors.
    """
    d = int(deg_q)
    ...
    # Binary unipotent sector: even chains with a nonzero diagonal form can be
    # represented at half length; the remaining chains require their full length.
...
def compute_sector_profile(F: np.ndarray, ctx, p: int) -> Dict[str, object]:
    ...
    try:
        ...
        if p == 2 and ctx.sector_type == "self" and is_x_pm_1(q, p) and Omega_sec is not None:
            ...
            from .mqf_unipotent_p2 import _p2_length_form_invariants
            for L in lengths:
                if L % 2 == 0:
