...
from sympleq.core.symmetries.modular_helpers import mod_p, omega_matrix, inv_mod_mat, rank_mod
from .mqf_certification import verify_cost_certificate
from .mqf_linear import split_uv
...
def _block_frame(blocks: Sequence[MQFBlock], n2: int, p: int) -> np.ndarray:
    """
    Reconstruct the global Darboux frame used by minimal_qudit_frame:
...
def _block_index_sets(blocks: Sequence[MQFBlock]) -> List[np.ndarray]:
    """
    Indices of each MQF block inside the global frame ordering
    [U_1, ..., U_r, V_1, ..., V_r].
    """
...
def verify_block(F: np.ndarray, T_blk: np.ndarray, p: int, *, check_invariant: bool = True) -> Dict[str, Any]:
    """
    Verify one returned MQF block basis.
...
def verify_block_family(F: np.ndarray, blocks: Sequence[MQFBlock], p: int) -> Dict[str, Any]:
    """Verify all MQF blocks and their pairwise symplectic orthogonality."""
    F = mod_p(np.asarray(F, dtype=np.int64), p)
    n2 = F.shape[0]
    Omega = _omega_for_dim(n2, p)
    ...
    for i in range(len(blocks)):
        ...
        for j in range(i + 1, len(blocks)):
            ...
            if np.any(gij % p):
                ...
                raise RuntimeError(
                    "MQF blocks are not symplectically orthogonal: "
...
def verify_sigma_block_diagonal(Sigma: np.ndarray, blocks: Sequence[MQFBlock], p: int) -> Dict[str, Any]:
    """
    ...
    for bi, idx in enumerate(idx_sets):
        ...
        if np.any(off_rows % p) or np.any(off_cols % p):
            raise RuntimeError(f"Sigma is not block diagonal with respect to MQF block {bi}.")

    return {"block_diagonal": True, "block_dims": [int(2 * b.half_dim) for b in blocks]}


def verify_minimal_qudit_frame(
    F: np.ndarray,
    B: np.ndarray,
    Sigma: np.ndarray,
    ...
    *,
    ...
    """End-to-end verification oracle for returned MQF decompositions."""
    ...
    if expect_full_block_cover and not block_cover:
        raise RuntimeError(
            "Returned B is not exactly the concatenated MQF block frame; "
