...
from .mqf_extension import GF2Extension
...
def _top_quotient_E_basis(F: np.ndarray, N: np.ndarray, L: int, deg_q: int, p: int) -> TopQuotient:
    ...
    for j in range(top_reps.shape[1]):
        ...
        trial = np.concatenate([base, selected_orbits, orbit], axis=1) if (
            base.shape[1] or selected_orbits.shape[1]) else orbit
...
def _hermitian_decompose(
    field: GF2Extension,
    H: np.ndarray,
) -> Tuple[List[Tuple[str, List[int], Optional[List[int]]]], Dict[str, Any]]:
    """
            ...
            if pair_for_line is not None:
                ...
                if w_line is None or nrm == 0:
                    raise RuntimeError(
                        ...
                        "after exhaustive field-coefficient search; "
...
def mqf_blocks_in_self_sector_p2_nonunipotent_unitary(
    F_sec: np.ndarray,
    T_sec: np.ndarray,
    Omega: np.ndarray,
    N: np.ndarray,
    ...
    p = int(p)
    if p != 2:
        raise ValueError("mqf_blocks_in_self_sector_p2_nonunipotent_unitary is p=2 only")
    if int(deg_q) <= 1:
    ...
    inv = MQFInvariant(sector_key=sector_key, sector_type="self", poly_key=poly_key, data=inv_data)
    return blocks, inv
