...
if __name__ == "__main__":
    ...
    print(f'Checking on d={d} toric code with nx={nx}, ny={ny}, periodic={periodic}')

    # print(f'Checking on Ising Ladder with ny= {ny}, and periodic= {periodic}')
    # hamiltonian =  ising_2d_hamiltonian(
    #     n_x=2,
    #     n_y=ny,
    #     J_zz=1.0,
    #     h_x=0.5,
    ...
    # hamiltonian = all_to_all_heisenberg_hamiltonian(
    #     n=ny,
    #     J=1.0,
    #     delta_z=0.5,
    # )

    S_mod = hamiltonian.symplectic_product_matrix()
    print(S_mod)
    vertex_colors = compress_vertex_colors(hamiltonian.weights)
    print('Building with Subdividion graph')
    # print(f'S_mod.shape: {S_mod.shape}')
    colors, _ = _build_base_partition(
        S_mod,
        int(hamiltonian.lcm),
        coeffs=hamiltonian.weights,
        col_invariants=None,
        max_rounds=0,
        color_mode="none",
    )
    graph_edges, vertex_colors_edges= build_subdivision_graph_from_s_mod(S_mod, colors)
    print(f'graph_edges: {graph_edges.summary()}')
    print(hamiltonian)
    if visualize:
        matplotlib.use("Agg")
        ...
        print_input_and_bliss_optimized_nodes(graph_edges, vertex_colors_edges)
        save_graph_files(graph_edges, f"d{d}_toric_edge_to_vertex", nx, ny)
