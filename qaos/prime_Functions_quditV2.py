...
from prime_Functions_Andrew import (
    int_to_bases, bases_to_int, string_to_pauli, old_string_to_pauli,
    weighted_vertex_covering_maximal_cliques, ground_state, 
    commutation_graph, graph, pauli_product, pauli_to_string, 
    diagonalize, act, act_inverse, quditwise_commutation_graph, quditwise_inner_product, 
    pauli_to_matrix, pauli, Hamiltonian_Mean, scale_variances, circuit, gate,
    H, CX, S, CZ, anticommutation_graph, symplectic_inner_product, print_Ham_string
)
...
def bfq_experiment_initial(P,cc,pauli_block_sizes,shots,general_commutation = True, 
    ...
    return(xxx,circuit_list,(P,cc,pauli_block_sizes,X,S,D,CG,aaa,k_phases,general_commutation,allocation_mode,N_chain,N_mcmc,N_mcmc_max,mcmc_shot_scale,shots,circuit_list,xxx))
...
def bfq_experiment(xxx,rr,shots,algorithm_variables):
    ...
    return(xxx,circuit_list,(P,cc,pauli_block_sizes,X,S,D,CG,aaa,k_phases,general_commutation,allocation_mode,N_chain,N_mcmc,N_mcmc_max,mcmc_shot_scale,shots_total,circuit_list_total,xxx_total))
...
def bfq_estimation(xxx,rr,algorithm_variables):
    ...
    error_estimate = np.sqrt(np.sum(scale_variances(graph(bayes_covariance_graph(X,np.array(cc),CG.adj,p,pauli_block_sizes,int(P.lcm), N_chain = N_chain, N = N_mcmc + int(shots_total * mcmc_shot_scale), N_max = N_mcmc_max + 4*int(shots_total * mcmc_shot_scale))),S).adj)).real
    
    return(mean,error_estimate,(P,cc,pauli_block_sizes,X,S,D,CG,aaa,k_phases,general_commutation,allocation_mode,N_chain,N_mcmc,N_mcmc_max,mcmc_shot_scale,shots_total,circuit_list_total,xxx_total))
...
def read_Hamiltonian(path):
    ...
    return old_string_to_pauli(sss1),cc
...
def ground_set(M):
    #
    return list((M.T).dot(1 << np.arange(M.shape[0] - 1, -1, -1)))
...
def gf2_rank(cols):
    ...
    while cols:
        ...
        if pivot_col:
            rank += 1
            lsb = pivot_col & -pivot_col
            for index, col in enumerate(cols):
                if col & lsb:
                    cols[index] = col ^ pivot_col
    return rank

def symplectic_RREF(P):
    q = P.qudits()
    P1 = P.copy()
    C = circuit(q)
    pivots = []
    ...
    for i in range(q):
        C,pivots = symplectic_RREF_iter_(P1.copy(),C,pivots,i)
    P1 = act(P1,C)
...
def symplectic_RREF_iter_(P,C,pivots,a):
    p,q = P.paulis(),P.qudits()
    P = act(P,C)
    ...
    if any(P.X[:,a]) or any(P.Z[:,a]):
        ...
        if any(P.Z[a1,i] for i in range(a+1,q)):
            # gg = [gate(CZ,[a,i]) for i in range(a+1,q) if P.Z[a1,i]]
            gg = [gate(H,[a])]
...
def reduce_qubits(P):
    C,pivots = symplectic_RREF(P)
    P1 = act(P,C)
    P1.delete_paulis_(set(range(P1.paulis()))-set([pivot[0] for pivot in pivots]))
    M = anticommutation_graph(P1).adj
    dim = M.shape[0]
    rank = gf2_rank(ground_set(M))
    # P1.print_symplectic()
    # print()
    # print("# Qubits: ",rank//2)
    # print("-"*12)
    # print("# Removed:",P1.qubits()-dim+rank//2)
    # print("# Repeats:",dim-rank)
    return C,pivots

def reduced_Hamiltonians(PP,ccc):
    PP1,ccc1 = [],[]
    for i in range(len(PP)):
        ...
        C,pivots = reduce_qubits(P)
        P1 = act(P,C)
        P1.print()
        bb = negations(P,C)
        cc_neg = [(-1)**bb[i]*cc[i] for i in range(p)]

        removable_qubits = set(range(q))-set([pivot[1] for pivot in pivots])
        conditional_qubits = set(range(q))-removable_qubits-set([pivot[1] for pivot in pivots if pivot[2]=='Z'])
        removable_qubits = sorted(removable_qubits)
        conditional_qubits = sorted(conditional_qubits)
        conditional_factors = []
        for i0 in range(p):
            conditional_factors.append([P1.Z[i0,i1] for i1 in conditional_qubits])
        P1.delete_qubits_(set(removable_qubits)|set(conditional_qubits))
        print("Q",q)
        print("R",len(removable_qubits))
        print("C",len(conditional_qubits))
        print("Removed",removable_qubits)
        print("Conditioned",conditional_qubits)
        return PP,ccc
        # P.print()
        # print()
        # C.print()
        ...
        for conditions in itertools.product([True,False],repeat=len(conditional_qubits)):
            cc1 = [cc_neg[i0]*(-1)**(sum(conditions[i1]*conditional_factors[i0][i1] for i1 in range(len(conditional_qubits)))) for i0 in range(p)]
            distinct_paulis = list(set([pauli_to_string(P1.a_pauli(i)) for i in range(P1.paulis())]))
            cc_dict = {}
            for ss in distinct_paulis:
                cc_dict[ss] = 0
            for i in range(P1.paulis()):
                cc_dict[pauli_to_string(P1.a_pauli(i))] += cc1[i]
            for ss,c in cc_dict.items():
                if ss != '':
                    if np.abs(c) < 10**(-10):
                        distinct_paulis.remove(ss)
            P2 = string_to_pauli(distinct_paulis)
            cc2 = []
            for i in range(P2.paulis()):
                cc2.append(cc_dict[pauli_to_string(P2.a_pauli(i))])
            PP1.append(P2)
            ccc1.append(cc2)
    return PP1,ccc1
...
def fast_rc(P):
    q = P.qubits()
    row_set = set(range(P.paulis()))
    M = np.concatenate([P.X,P.Z],axis=1)
    pivot_rows = set([])
    for col in range(2*q):
        if not any(M[r,col] for r in row_set-pivot_rows):
            continue
        row = min(r for r in row_set-pivot_rows if M[r,col])
        pivot_rows.add(row)
        for r in row_set:
            if M[r,col] and r != row:
                M[r,:] ^= M[row,:]
    pivot_rows = sorted(pivot_rows)
    C = np.array([[symplectic_inner_product(P.a_pauli(i0),P.a_pauli(i1)) for i1 in pivot_rows] for i0 in pivot_rows],dtype=bool)
    p = C.shape[0]
    for i in range(p):
        if not any(C[r,i] for r in range(i+1,p)):
            continue
        if i>0 and C[i-1,i]:
            for r in range(i+1,p):
                if C[r,i]:
                    ...
                    C[:,r] ^= C[:,i-1]
            continue
        i_min = min(r for r in range(i+1,p) if C[r,i])
        if i_min > i+1:
            C[i+1,:] ^= C[i_min,:]
            C[:,i+1] ^= C[i_min,:]
        for r in range(i+2,p):
            if C[r,i]:
                C[r,:] ^= C[i+1,:]
                C[:,r] ^= C[:,i+1]
    c_qubits = p-np.count_nonzero(C)
    r_qubits = q-(np.count_nonzero(C)//2)-c_qubits
    print("Q",q)
    print("R",r_qubits)
    print("C",c_qubits)
    return r_qubits,c_qubits
...
names = []
for name in ["4qubits_H2","8qubits_H2","12qubits_LiH","14qubits_BeH2","14qubits_H2O","16qubits_NH3","20qubits_HCl","20qubits_C2","32qubits_phenyl",]:
    form = "jw"
    print(name,form)

    file_Hamiltonian = str(root_path) + "/Hamiltonians/" + name + "/" + form + ".txt"

    P,cc = read_Hamiltonian(file_Hamiltonian)
    p,q = P.paulis(),P.qudits()
    ...
    # print_Ham_string(P,cc)
    # print()
    P1 = act(P,C)
    bb = negations(P,C)
    cc = [(-1)**bb[i]*cc[i] for i in range(p)]
    # print("Equivalent Hamiltonian:")
    # print_Ham_string(P1,cc)
    # print()
    ...
    for i0 in range(p):
        conditional_factors.append([P1.Z[i0,i1] for i1 in conditional_qubits])
    P1.delete_qudits_(set(removable_qubits)|set(conditional_qubits))
    print("Qudits:",P.qudits(),"->",P1.qudits())
    # print("Hamiltonians:",1<<len(conditional_qubits))
    Pz = old_string_to_pauli(["".join("Z" if j==i else "I" for j in range(q)) for i in conditional_qubits])
    Pz = act_inverse(Pz,C)
    ...
    for conditions in itertools.product([True,False],repeat=len(conditional_qubits)):
        cc1 = [cc[i0]*(-1)**(sum(conditions[i1]*conditional_factors[i0][i1] for i1 in range(len(conditional_qubits)))) for i0 in range(p)]
        distinct_paulis = list(set([pauli_to_string(P1.a_pauli(i))[0] for i in range(P1.paulis())]))
        cc_dict = {}
        for ss in distinct_paulis:
            cc_dict[ss] = 0
        for i in range(P1.paulis()):
            cc_dict[pauli_to_string(P1.a_pauli(i))[0]] += cc1[i]
        tbr = []
        for ss,c in cc_dict.items():
            if np.abs(c) < 10**(-10):
                distinct_paulis.remove(ss)
        P2 = old_string_to_pauli(distinct_paulis)
        cc2 = []
        for i in range(P2.paulis()):
            cc2.append(cc_dict[pauli_to_string(P2.a_pauli(i))[0]])
        # psi2 = ground_state(P2,cc2)
        # print("GSE:",Hamiltonian_Mean(P2,cc2,psi2))
        # print_Ham_string(P2,cc2)
        # print()
