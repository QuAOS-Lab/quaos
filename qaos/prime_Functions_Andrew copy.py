...
def act_inverse(P,C):
    ...
    if P == None:
        ...
        return act_inverse(P,C.gg)
    elif len(C) == 0:
        return P
    elif len(C) == 1:
        return act_inverse(P,C[0])
    else:
        return act_inverse(act_inverse(P,C[-1]),C[:-1])
...
# returns the anticommutation graph of a given Pauli
def anticommutation_graph(P):
    # Inputs:
    #     P - (pauli) - Pauli to check for commutation relations
    # Outputs:
    #     (graph) - an edge is weighted 1 if the pair of Paulis anticommute
    p = P.paulis()
    C = np.array([[symplectic_inner_product(P.a_pauli(i0), P.a_pauli(i1)) for i1 in range(p)] for i0 in range(p)], dtype=bool)
    C[range(p),range(p)] = False
    return graph(C, dtype=bool)
...
def print_Ham_string(P,cc):
    ...
    for i in range(P.paulis()):
        print(pauli_to_string(P.a_pauli(i))[0],end="")
        print('',cc[i])
