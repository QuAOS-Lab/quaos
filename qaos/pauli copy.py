...
# convert IXYZ string to Pauli
def old_string_to_pauli(sss):
    # Inputs:
    #     sss - (list{str}) or (str) - string representation of Pauli
    # Outputs:
    #     (pauli) - Pauli corresponding to input string(s)
...
def pauli_to_string(P):
    ...
    if P.lcm == 2:
        ...
        if P.paulis() == 0:
            return '',P.dims,P.phases
        elif P.paulis() == 1:
            return ''.join(ssDict[(X[0,i],Z[0,i])] for i in range(P.qudits())),P.dims,P.phases
        else:
            return [''.join(ssDict[(X[i0,i1],Z[i0,i1])] for i1 in range(P.qudits())) for i0 in range(P.paulis())],P.dims,P.phases
    else:
