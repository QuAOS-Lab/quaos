...
def check_mappable_via_clifford(pauli_sum: PauliSum, target_pauli_sum: PauliSum) -> bool:
    ...
    if pauli_sum.dimensions != target_pauli_sum.dimensions:
    ...
    return pauli_sum.symplectic_product_matrix() == target_pauli_sum.symplectic_product_matrix()


def find_circuit(pauli_sum: PauliSum, target_pauli_sum: PauliSum, iterations: int,
                 compare_phases: bool = True, stop_if_found: bool = True) -> Circuit | list[Circuit] | None:
...
def find_agnostic_circuit(pauli_sum: PauliSum, target_paulis: list[Pauli], target_indexes: list[tuple[int, int]],
                          iterations: int, compare_phases: bool = True,
                          stop_if_found: bool = True) -> Circuit | list[Circuit] | None:
    """
    ...
    """
    num_free_paulis = pauli_sum.n_paulis() * pauli_sum.n_qudits() - len(target_paulis)

    current_target = pauli_sum.copy()
    for i, indexes in enumerate(target_indexes):
        current_target[indexes[0], indexes[1]] = target_paulis[i]  # make __setitem__ work
    ...
    if not mappable:
        ...
        C = find_circuit(pauli_sum, current_target, iterations, compare_phases, stop_if_found)


if __name__ == "__main__":
    initial_pauli = ['x1z1 x1z0', 'x0z0 x0z1']
    goal_pauli = ['x1z1 x1z0', 'x0z1 x0z0']
