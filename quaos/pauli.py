...
class pauli:
    def __init__(self, X, Z, dims=2, phases=0):
        ...
        ----------
        X : numpy.array
            X-part of Pauli in symplectic form with shape (p,q)
        Z : numpy.array
            Z-part of Pauli in symplectic form with shape (p,q)
        dims : int or list of ints, optional
            The dimensions of each of the qudits. If int, this is the dimension of each qudit.
            If list, this is the dimension of each qudit.
        phases : int or list of ints, optional
            The phases of each of the Paulis. If int, this is the phase of each Pauli.
            If list, this is the phase of each Pauli.
        """
        if X.shape != Z.shape:
    ...
    def print_symplectic(self):
        ...
        for i in range(self.paulis()):
            print(''.join(str(int(i1)) for i1 in self.X[i, :]) + ' ' + ''.join(str(int(i1)) for i1 in self.Z[i, :]) +
                  ' ' + str(self.phases[i]) + '/' + str(self.lcm))


# convert a pauli object to its matrix representation
def pauli_to_matrix(P):
    """
    Convert a generalised Pauli object (clock and shift) to its matrix representation.

    Parameters
    ----------
    P : pauli
        Pauli object to be converted to matrix representation. Must have shape (1,q).
    ...
    ------
    Exception
        If the Pauli object to be converted is not a single Pauli operator (i.e., does not have shape (1,q)).
    """
    if P.paulis() != 1:
...
if __name__ == "__main__":
    ...
    P3 = symplectic_inner_product(P, P2)    
    print(P3)
