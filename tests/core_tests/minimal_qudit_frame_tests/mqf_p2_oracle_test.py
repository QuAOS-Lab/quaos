"""
Independent verification of the p=2 MQF decomposition.

Three layers:
  * relation and distinction tests: conjugate presentations must give equal
    qudit cost, and the W/V dichotomy must separate the classes it should;
...
Runs under pytest, or standalone via ``python3 mqf_p2_oracle_test.py`` (the module
...
from sympleq.core.minimal_qudit_frame.helpers._brute_force_oracle import (
    brute_force_cost,
    OracleInfeasible,
)
...
# --------------------------------------------------------------------------- #
def _decompose(F):
    Sigma, Bm, info = minimal_qudit_frame(F, P)
...
def _fingerprint(info):
    ...
    return (
        _cost(info),
        tuple(sorted(info.get("mqf_half_dims", []))),
        tuple(sorted(fam_count.items())),
    )


# --------------------------------------------------------------------------- #
# Relation and distinction tests.
# --------------------------------------------------------------------------- #
...
def test_oracle_odd_p_conjugation_invariance():
    ...
    for name, F0, p in cases:
        ...
        for seed in (1, 2, 3):
            ...
            Sigma, Bm, info = minimal_qudit_frame(Fc, p)
...
# (mqf_self_p2_unitary) for p=2, or the odd-p Witt builder (mqf_self)
...
def test_oracle_self_p2_unitary():
    """p=2 Hermitian self-sector (mqf_self_p2_unitary): pipeline cost, lower
    ...
    Sigma, Bm, info = minimal_qudit_frame(F, 2)
    ...
    for seed in (1, 2, 3):
        ...
        Sigma2, Bm2, info2 = minimal_qudit_frame(Fc, 2)
...
def test_oracle_self_witt_generic_basis():
    """Odd-p self-reciprocal self-sector (mqf_self / Witt path) on generic
