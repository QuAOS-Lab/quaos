...
from sympleq.core.minimal_qudit_frame.helpers.mqf_types import SectorCostCertificate
...
def test_phase_c_sector_certificates_have_typed_and_legacy_views() -> None:
    ...
    Sigma, B, info = minimal_qudit_frame(F, 2)
...
def test_phase_c_p2_unipotent_uses_quotient_normal_form_not_enumeration() -> None:
    ...
    assert sorted(int(h) for h in info["mqf_half_dims"]) == sorted([3, 2, 5, 1])
    assert info["Q_opt"] == 5
