...
from sympleq.core.minimal_qudit_frame import MinimalQuditFrameCertificationError, minimal_qudit_frame
    ...
    *,
    ...
    try:
        ...
        pytest.fail(
            ...
            f"MinimalQuditFrameCertificationError.info={pprint.pformat(exc.info, width=140)}"
        )
    ...
    assert Q_cost == max(int(h) for h in info["mqf_half_dims"])
    ...
    #     f"mqf_half_dims={info['mqf_half_dims']}\n"
    #     f"trial_meta={pprint.pformat(trial_meta, width=120)}"
    # )
    if Q_cost > int(Q_max):
        sector_payload = []
...
class TestMinimalQuditFrameDecompositionRandomBoundedCost:
...
def test_scrambled_direct_sum_of_one_qudit_blocks_has_cost_one() -> None:
    ...
    Sigma, B, info = minimal_qudit_frame(F_scrambled, p)
    verify_global_basis(F_scrambled, B, Sigma, p)

    assert int(info["Q_opt"]) == 1
    assert sorted(int(h) for h in info["mqf_half_dims"]) == [1] * 8
