...
class TestDecomposition:
    ...
    def test_preconditioner_after_random_conjugation(self):
        ...
        for p in [2, 3, 5]:
            for _ in range(num_trials):
                ...
                C = Circuit.from_random(depth=depth, dimensions=[p] * n)
    ...
    def test_phase_correction(self):
        ...
        for p in [2, 3, 5]:
            for _ in range(num_trials):
                C_in = Circuit.from_random(depth=n_gates_in_C_in, dimensions=[p] * n)
    ...
    def test_gate_to_circuit_roundtrip_test(self):
        ...
        for p in (2, 3, 5):
            for _ in range(num_trials):
                C_in = Circuit.from_random(depth=12, dimensions=[p] * n)
